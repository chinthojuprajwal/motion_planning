...
for i = 2:nsamples
    ...
    %
    [sorted_dist,arg_sort]=sort(distances);
    if length(arg_sort)>=k
        arg_sort=arg_sort(1:k);
        sorted_dist=sorted_dist(1:k);
    end
    for i1=1:length(arg_sort)
        if LocalPlanner(x,samples(:,arg_sort(i1)))
            edges(nedges+1,:)=[i,arg_sort(i1)];
            edge_lengths(nedges+1)=sorted_dist(i1);
            nedges=nedges+1;
        end
    end
